...
用法:
    ...
    python unified_crawler.py cninfo-search <关键词> --step all --date 2024-01-01~2024-12-31
    python unified_crawler.py cninfo-search 年报 --step index --stock 000001 --category 年报
"""
...
class CninfoSearchDownloader:
    ...
    PAGE_SIZE_MAX = 30
    MAX_PAGES_PER_QUERY = 100  # 巨潮接口单次查询最大页数限制（超出会截断）
    ...
    INDEX_FIELDS = [
        ...
        'index_created_at',
        'query_stock_input',
        'query_stock_resolved',
        'query_category_input',
        'query_category_resolved',
        'query_column',
        'query_plate',
        'query_start_date',
        'query_end_date',
    ...
    def __init__(self, output_dir: str = CNINFO_OUTPUT_DIR):
        ...
        self.top_search_url = 'https://www.cninfo.com.cn/new/information/topSearch/query'
        self.announcement_download_url = 'https://www.cninfo.com.cn/new/announcement/download'
        self.headers = {
    ...
    def _normalize_page_size(page_size: int) -> int:
        try:
            ...
            page_size = 30
        return max(1, min(page_size, CninfoSearchDownloader.PAGE_SIZE_MAX))

    @staticmethod
    def _max_records_per_query(page_size: int) -> int:
        return max(1, int(page_size)) * CninfoSearchDownloader.MAX_PAGES_PER_QUERY

    @staticmethod
    def _get_total_record_num(data: dict) -> int:
        if not isinstance(data, dict):
            return 0
        for key in ('totalRecordNum', 'totalAnnouncement'):
            v = data.get(key)
            if v is None:
                continue
            try:
    ...
    def resolve_stock(self, stock_input: str) -> str:
        """
        通过 topSearch 接口将股票代码/简称解析为 "code,orgId" 格式。
        解析失败时返回原始输入。
        """
        stock_input = self._safe_text(stock_input)
        if not stock_input:
            return ''
        if ',' in stock_input:
            return stock_input
        ...
        try:
            resp = self.session.post(self.top_search_url, data={'keyWord': stock_input}, timeout=10)
            resp.raise_for_status()
            results = resp.json()
            if isinstance(results, list) and results:
                item = results[0] or {}
                code = self._safe_text(item.get('code'))
                org_id = self._safe_text(item.get('orgId'))
                zwjc = self._safe_text(item.get('zwjc'))
                if code and org_id:
                    if zwjc:
                        print(f"    [cninfo-search] 股票已匹配: {code} {zwjc} (orgId={org_id})")
                    else:
                        print(f"    [cninfo-search] 股票已匹配: {code} (orgId={org_id})")
                    return f"{code},{org_id}"
        except Exception as e:
            print(f"    [cninfo-search] ⚠️ 股票代码解析失败: {stock_input} ({e})")

        return stock_input

    def resolve_category(self, category_input: str) -> str:
        raw = self._safe_text(category_input)
        if not raw:
            return ''
        if raw in CNINFO_CATEGORIES:
            return CNINFO_CATEGORIES[raw]
        if raw.startswith('category_'):
            return raw
        print(f"    [cninfo-search] ⚠️ 未知分类: {raw} (将忽略 category)")
        return ''

    def _build_payload(self, keyword: str, start_date: str, end_date: str,
                       page_no: int, page_size: int,
                       stock: str = '', category: str = '',
                       column: str = 'szse', plate: str = '') -> dict:
        return {
            'pageNum': str(page_no),
    ...
    def _fetch_page(self, keyword: str, start_date: str, end_date: str,
                    page_no: int, page_size: int,
                    stock: str = '', category: str = '',
                    column: str = 'szse', plate: str = '') -> dict:
    ...
    def _normalize_record(self, item: dict, keyword: str, index_created_at: str,
                          query_ctx: dict) -> dict:
        ...
        return {
            ...
            'query_stock_input': self._safe_text(query_ctx.get('stock_input')),
            'query_stock_resolved': self._safe_text(query_ctx.get('stock_resolved')),
            'query_category_input': self._safe_text(query_ctx.get('category_input')),
            'query_category_resolved': self._safe_text(query_ctx.get('category_resolved')),
            'query_column': self._safe_text(query_ctx.get('column')),
            'query_plate': self._safe_text(query_ctx.get('plate')),
            'query_start_date': self._safe_text(query_ctx.get('start_date')),
            'query_end_date': self._safe_text(query_ctx.get('end_date')),
        }

    def _collect_range(self, keyword: str, start_date: str, end_date: str,
                       page_size: int, max_pages: int, max_results: int,
                       stock: str, category: str, column: str, plate: str,
                       created_at: str, query_ctx: dict,
                       seen_keys: set,
                       first_page_data: dict = None) -> list:
        records = []
        page_no = 1
        ...
        while True:
            ...
            total_record_num = self._get_total_record_num(data)
            ...
            for item in announcements:
                record = self._normalize_record(item, keyword, created_at, query_ctx=query_ctx)
            ...
            print(f"页 {page_no}: 本页 {len(announcements)} 条, 新增 {page_added} 条, "
                  f"累计 {len(records)} 条, 接口总量 {total_record_num}")
            ...
            if max_pages and page_no >= max_pages:
                ...
                break
            if page_no >= self.MAX_PAGES_PER_QUERY:
                print(f"达到接口单次查询页数上限 {self.MAX_PAGES_PER_QUERY}，停止抓取。")
                break
            if not has_more:
    ...
    def _search_segmented(self, keyword: str,
        ...
        try:
            ...
            print(f"❌ 分段查询失败: {start_date}~{end_date} ({e})")
            return []

        total = self._get_total_record_num(first)
        limit = self._max_records_per_query(page_size)
        if total == 0:
            return []

        if total <= limit or start_dt >= end_dt:
            print(f"分段 {start_date}~{end_date}: total={total} (<= {limit})")
            return self._collect_range(
                ...
                query_ctx=query_ctx,
                seen_keys=seen_keys,
                first_page_data=first,
            )

        # 需要继续二分拆分
        mid = start_dt + (end_dt - start_dt) / 2
        mid = datetime(mid.year, mid.month, mid.day)
        next_start = mid + timedelta(days=1)

        print(f"分段 {start_date}~{end_date}: total={total} (> {limit})，二分为 "
              f"{start_date}~{mid.strftime('%Y-%m-%d')} + {next_start.strftime('%Y-%m-%d')}~{end_date}")

        left = self._search_segmented(
            keyword=keyword,
            start_dt=start_dt,
            end_dt=mid,
            page_size=page_size,
    ...
    def search_and_build_index(self, keyword: str, start_date: str, end_date: str,
                               ...
                               stock: str = '', category: str = '',
                               column: str = 'szse', plate: str = '') -> list:
        ...
        if max_pages is not None:
            try:
                ...
                max_pages = None
        if max_pages:
            max_pages = min(max_pages, self.MAX_PAGES_PER_QUERY)

        stock_input = self._safe_text(stock)
        stock_resolved = self.resolve_stock(stock_input) if stock_input else ''

        category_input = self._safe_text(category)
        category_resolved = self.resolve_category(category_input)

        column = self._safe_text(column)
        plate = self._safe_text(plate)

        print(f"\n=== [cninfo-search] 开始检索 ===")
        print(f"关键词: {keyword}")
        if stock_input:
            print(f"股票: {stock_input} -> {stock_resolved}")
        if category_input:
            print(f"分类: {category_input} -> {category_resolved or '(忽略)'}")
        print(f"范围: {start_date} ~ {end_date}")
        print(f"交易所/市场(column): {CNINFO_COLUMNS.get(column, column) or '(空=全部)'}")
        if plate:
            print(f"板块(plate): {plate}")
        ...
        # 探测总量（用于判断是否需要自动分段，避免 100 页上限截断）
        first_data = None
        total = 0
        try:
            first_data = self._fetch_page(
                keyword=keyword,
                start_date=start_date,
                end_date=end_date,
                page_no=1,
            ...
            total = self._get_total_record_num(first_data)
        except Exception as e:
            print(f"❌ 第 1 页请求失败: {e}")
            return []

        limit = self._max_records_per_query(page_size)
        if max_pages is None and max_results is None and total > limit:
            # 自动二分日期范围，尽可能保证抓全（最小分到单日）
            print(f"⚠️ 记录数({total})超过单次查询上限({limit})，将自动按日期分段抓取以避免截断。")
            st = self._parse_date(start_date)
            ed = self._parse_date(end_date)
            all_records = self._search_segmented(
                keyword=keyword,
                start_dt=st,
                end_dt=ed,
                page_size=page_size,
    ...
    def _download_file(self, url_candidates, save_path: str, max_retries: int = 3) -> tuple:
        if isinstance(url_candidates, str):
            url_candidates = [url_candidates]
        url_candidates = [u for u in (url_candidates or []) if str(u or '').strip()]
        ...
        for url in url_candidates:
            ...
            # 当前候选 URL 失败，继续尝试下一个
        return False, (err or f"所有URL均失败(last={last_url})")
    ...
    def download_from_index(self, index_path: str = None, max_workers: int = MAX_DOWNLOAD_WORKERS) -> str:
        ...
        for record in records:
            ...
            primary_url = str(record.get('download_url_static') or record.get('adjunct_url') or '').strip()
            ...
            if os.path.exists(save_path) and os.path.getsize(save_path) > 1024:
                report_rows.append({
                    ...
                    'sec_name': str(record.get('sec_name') or ''),
                    'announcement_date': str(record.get('announcement_date') or ''),
                    'download_url_static': primary_url,
                    'local_filename': local_filename,
                    'status': 'skip',
                    'error': '',
                    'file_path': save_path,
            ...
            url_candidates = [primary_url]
            if primary_url.startswith('http://'):
                url_candidates.append(primary_url.replace('http://', 'https://', 1))
            ann_id = str(record.get('announcement_id') or '').strip()
            if ann_id:
                url_candidates.append(f"{self.announcement_download_url}?bulletinId={ann_id}&realTime=true")

            download_tasks.append((record, url_candidates, local_filename, save_path, primary_url))
        ...
        if download_tasks:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(self._download_file, url_candidates, save_path): (
                        record, url_candidates, local_filename, save_path, primary_url
                    )
                    for record, url_candidates, local_filename, save_path, primary_url in download_tasks
                }
                iterator = (tqdm(as_completed(futures), total=len(futures), desc='cninfo下载')
                            if HAS_TQDM else as_completed(futures))

                for future in iterator:
                    record, url_candidates, local_filename, save_path, primary_url = futures[future]
                    ok, msg = future.result()
                    report_rows.append({
                        'announcement_id': str(record.get('announcement_id') or ''),
                        'sec_code': str(record.get('sec_code') or ''),
    ...
    def _parse_date_range(date_range: str) -> tuple:
        ...
        if '~' not in date_range:
            raise ValueError("date 格式错误，应为 YYYY-MM-DD~YYYY-MM-DD")
    ...
    def run(self, keyword: str, step: str = 'index', start_date: str = None, end_date: str = None,
        ...
        if date_range:
            d_start, d_end = self._parse_date_range(date_range)
            start_date = d_start
            end_date = d_end
...
def cmd_cninfo_search(args):
    ...
    try:
        crawler.run(
            ...
            column=getattr(args, 'column', 'szse'),
            plate=getattr(args, 'plate', ''),
        )
...
示例:
  ...
  python unified_crawler.py cninfo-excel links.xlsx --col 0     # 从 Excel 链接列下载
    ...
    p_csearch.add_argument('keyword', help='检索关键词（必填）')
    p_csearch.add_argument('--step', choices=['index', 'download', 'all', 'crawl'],
                           default='index', help='执行步骤 (默认: index；crawl 为 index 的别名)')
    p_csearch.add_argument('-o', '--output', default=None, help=f'输出目录 (默认: {CNINFO_OUTPUT_DIR})')
    p_csearch.add_argument('--stock', '-s', default='',
                           help='股票代码或简称（可选，如 000001、平安银行；会自动解析为 code,orgId）')
    p_csearch.add_argument('--category', '-c', default='',
                           help='公告分类（可选：年报|半年报|一季报|三季报|IPO|增发|配股|债券；或直接传 category_...）')
    p_csearch.add_argument('--column', default='szse',
                           help='交易所/市场（默认: szse；传空字符串表示全市场）')
    p_csearch.add_argument('--plate', default='',
                           help='板块（可选，如 shkcp=科创板、szcy=创业板 等）')
    p_csearch.add_argument('--date', '-d', default=None,
                           help='日期范围快捷写法 YYYY-MM-DD~YYYY-MM-DD（优先于 --start-date/--end-date）')
    ...
    p_csearch.add_argument('--max-results', dest='max_results', type=int, default=None, help='最大记录数 (默认: 不限制)')
    p_csearch.add_argument('--max', dest='max_results', type=int, default=None, help='同 --max-results（兼容）')
    p_csearch.add_argument('--index', dest='index', default=None, help='索引文件路径（download 步骤使用）')
    p_csearch.add_argument('--json', dest='index', default=None, help='同 --index（兼容）')
